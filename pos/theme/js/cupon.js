...
$('#save,#update').click(function (e) {
    ...
    if(this_id=="save")  //Save start
    {
					if(confirm("Do You Wants to Save Record ?")){
						e.preventDefault();
						data = new FormData($('#cupon-form')[0]);//form name
						/*Check XSS Code*/
             ...
             // alert(result);return;
							if(result=="success")
							{
								...
								window.location=base_url+"cupon/view";
								return;
							}
	...
	else if(this_id=="update")  //Save start
    {
					...
					if(confirm("Do You Wants to Update Record ?")){
						...
						$.ajax({
						type: 'POST',
						url: base_url+'cupon/update_cupon',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
...
//Delete Record start
function delete_cupon(q_id)
{

   if(confirm("Do You Wants to Delete Record ?")){
   	$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post("delete_cupon",{q_id:q_id},function(result){
   //alert(result);return;
	   if(result=="success")
				{
