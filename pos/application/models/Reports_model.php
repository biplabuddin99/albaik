...
class Reports_model extends CI_Model {
	...
	public function show_stock_report(){
		...
		$this->db->select("a.*,(select sum(qty) from db_stockentry where db_stockentry.item_id=a.id) as stockr,b.tax_name");
...
//		$this->db->group_by("c.`item_id`");
	...
	public function statement($type,$startDate,$endDate,$paytype=false){
		...
		//Getting Expenses
		$this->db->select('id,expense_code,expense_date,expense_amt');
		$this->db->from('db_expense');
